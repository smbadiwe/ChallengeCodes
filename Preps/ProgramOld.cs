...
namespace Preps
{
    ...
    partial class ProgramOld
    {
        static int mfibDP(int n)
        {
            if (n <= 2) return 1;
        ...
        public static Tuple<int, int> FindTwwoSum(List<int> input, int number)
        {
            if (input == null) return null;
            if (input.Count < 2) // i.e.
            {
                // We're looking for zero-based indices of ANY TWO distinct elements whose sum is equal to the target sum
                return null;
            }
            input.Sort();
            int end = input.Count - 1;
            // ignore number that are >= the given sum
            while (end >= 0 && input[end] >= number)
            {
                end--;
            }
            if (end < 0)
        ...
        static public Tuple<int, int> FindTwoSum(List<int> input, int number)
        {
            //write code here 
        ...
        {
            var ans = FindTwoSum(new List<int>() { 2, 5, 3, 6, 12 }, 15);
